	...
	this.init = function() {    	
        ...
        */
		$("#btn_quit").click(function() {
			...
			location.href = "commentlist.html?isRefresh=1&id=" + id;
		});
		...
		$("#btn_send")
				.click(
						function() {
							...
							if (length == 0)
								inputBox
										.html("<span id='used'>请输入微博内容！</span>");
							else if (length > max) {

							} else {
								gm.ngi.weibosdk.api.iComment.apiComplete = function(
										ahr) {
									if (ahr.succeeded) {
										if (ahr && ahr.data && ahr.data.status) {
											var postParam = {
													id:ahr.data.status.id,
													reurl :pageParams.reurl2
												};
											//var reurl = pageParams.reurl;
											//location.href = "statusdetail.html?isRefresh=1&id="
											//		+ ahr.data.status.id;
								...
								gm.ngi.weibosdk.api.iComment.create(content,
										id, 0);
							}
						});
		$("#txt_content").keyup(
				function() {
					...
					if (length > max) {
						...
						// textBox.val(con);
						inputBox.html("已经输入超过<span id='used'>"
								+ (length - max) + "</span>个字");
					} else
						inputBox.html("还可以输入<span id='used'>"
								+ (max - length) + "</span>个字");
				});
		//$("#txt_content").focus();
	};
